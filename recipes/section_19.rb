...
powershell_script 'third-party_content' do
  code <<-EOH
    $sid=((New-Object System.Security.Principal.NTAccount('local-admin')).Translate([System.Security.Principal.SecurityIdentifier]).Value); ($regpath = "Registry::HKEY_USERS\\$sid\\Software\\Policies\\Microsoft\\Windows\\CloudContent"); New-Item $regpath -force | New-ItemProperty -Name 'DisableThirdPartySuggestions' -Value '1' -PropertyType dword -Force |Out-Null
  EOH
  only_if '$sid=((New-Object System.Security.Principal.NTAccount("local-admin")).Translate([System.Security.Principal.SecurityIdentifier]).Value); (Get-ItemProperty "Registry::HKEY_USERS\\$sid\\Software\\Policies\\Microsoft\\Windows\\CloudContent" -name "DisableThirdPartySuggestions") -eq $null'
end
